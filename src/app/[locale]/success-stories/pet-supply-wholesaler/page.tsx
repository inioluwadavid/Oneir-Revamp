...
export async function generateMetadata({
  ...
  return {
    title: isEnglish ? "Pet Supply Wholesaler | Success Stories" : "Grossiste en fournitures pour animaux | Histoires de succès",
...
export default async function PetSupplyWholesalerCaseStudy({ params }: CaseStudyPageProps) {
  const { locale: localeParam } = await params;
  const locale = localeParam as Locale;
  const t = getTranslations(locale);

  const caseStudy = t.caseStudies?.petSupplyWholesaler as {
  ...
  const title = locale === "en" ? "Pet Supply Wholesaler" : "Grossiste en fournitures pour animaux";
